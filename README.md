...
입력 디바운스를 여러 값으로 실험해본 결과, 250ms가 가장 자연스러운 지연 없이 안정적이었습니다. 또한 결과 개수도 50개로 상한을 두고 입력 최소 길이를 2자로 제한했습니다.
...
리스트 항목을 클릭하는 순간 바로 값이 선택되어 사용자가 의도하지 않은 기업이 적용되는 일이 있었고모달을 닫을 때 상태가 중간에 남아 혼란을 주었습니다.

**해결 방안**

클릭 시에는 내부적으로 `pending` 상태만 저장하고 최종 선택은 “선택하기” 버튼으로 확정하도록 UX를 변경했습니다. 이후 닫기/X/외부 클릭 시에는 값을 모두 초기화하여 모달을 닫는 행위가 명확한 취소가 되도록 했습니다.
